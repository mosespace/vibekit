...
type Row = {
  feature: string;
  cna: string | boolean;
  vibekit: string | boolean;
};

const rows: Row[] = [
  {
    feature: "Initial scaffold",
    cna: "Blank Next.js project",
    vibekit: "Planned project: 4 generated files + locked stack",
  },
  { feature: "Tech stack picked for you", cna: false, vibekit: true },
  {
    feature: "Auth ready out of the box",
    cna: false,
    vibekit: "JB Better Auth UI · one command",
  },
  {
    feature: "Database + ORM configured",
    cna: false,
    vibekit: "Neon + Prisma v7 in Phase 1",
  },
  {
    feature: "Design system applied",
    cna: false,
    vibekit: "Customized design-style-guide.md per project",
  },
  { feature: "Phase-by-phase build plan", cna: false, vibekit: true },
  {
    feature: "Pre-built components for common needs",
    cna: false,
    vibekit: "JB Component Registry (auth, files, Stripe, tables)",
  },
  { feature: "Pre-deploy security audit", cna: false, vibekit: true },
  {
    feature: "Works with any AI coding agent",
    cna: "Yes, but agent has to figure out everything",
    vibekit: "Yes, with framework rules locked in",
  },
  {
    feature: "Time to first feature",
    cna: "Hours of setup before you can build",
    vibekit: "Phase 1 done in ~1–2 hours",
  },
  {
    feature: "Maintenance across projects",
    cna: "Each project drifts in its own direction",
    vibekit: "Same stack + patterns across every project",
  },
...
function Cell({ value }: { value: string | boolean }) {
  ...
  return (
    <span className="text-[13.5px] text-[color:var(--text-primary)]">
      {value}
    </span>
  );
}
        ...
        <article className="mx-auto max-w-5xl px-4 sm:px-6">
          ...
          <header className="mt-8 border-b border-[color:var(--border)] pb-10">
            ...
            <p className="mt-5 max-w-2xl text-[18px] leading-relaxed text-[color:var(--text-secondary)]">
              <code className="font-mono text-[15px] rounded border border-[color:var(--border)] bg-[color:var(--bg-elevated)] px-1.5 py-0.5">
                create-next-app
              </code>{" "}
              gives you a blank Next.js project every decision is still ahead of
              you.{" "}
              ...
              project: stack locked, auth ready, design system customized,
              components installed, security audited.
            </p>
          </header>
          ...
          <section className="mt-12 grid gap-4 sm:grid-cols-2">
            <div className="rounded-md border border-[color:var(--border)] bg-[color:var(--bg-elevated)] p-6">
              ...
              <ul className="mt-4 space-y-2 text-[14px] leading-relaxed text-[color:var(--text-primary)]">
                <li className="flex gap-2">
                  ...
                  You're learning Next.js and want to make every decision
                  yourself
                </li>
                <li className="flex gap-2">
                  <span className="text-[color:var(--text-tertiary)]">·</span>
                ...
                <li className="flex gap-2">
                  <span className="text-[color:var(--text-tertiary)]">·</span>
                  Your project doesn't need auth, payments, file uploads, or a
                  database
                </li>
                <li className="flex gap-2">
                  <span className="text-[color:var(--text-tertiary)]">·</span>
            ...
            <div className="rounded-md border border-[color:var(--accent)]/40 bg-[color:var(--accent-soft)] p-6">
              ...
              <ul className="mt-4 space-y-2 text-[14px] leading-relaxed text-[color:var(--text-primary)]">
                <li className="flex gap-2">
                  <span className="text-[color:var(--accent)]">✓</span>You're
                  shipping a real SaaS or production app
                </li>
                <li className="flex gap-2">
                  <span className="text-[color:var(--accent)]">✓</span>You want
                  auth, payments, file uploads, and email working in days, not
                  weeks
                </li>
                <li className="flex gap-2">
                  <span className="text-[color:var(--accent)]">✓</span>You use
                  AI coding agents and want consistent output
                </li>
                <li className="flex gap-2">
                  <span className="text-[color:var(--accent)]">✓</span>You build
                  multiple projects and want a repeatable workflow
                </li>
              </ul>
            </div>
          </section>
          ...
          <section className="mt-14 rounded-md border border-[color:var(--border)] bg-[color:var(--bg-elevated)] p-8">
            ...
            <p className="mt-3 text-[15.5px] leading-relaxed text-[color:var(--text-secondary)]">
              VibeKit doesn't replace{" "}
              <code className="font-mono text-[13.5px] rounded border border-[color:var(--border)] bg-[color:var(--bg-subtle)] px-1.5 py-0.5">
                create-next-app
              </code>{" "}
              it sits on top of it. Phase 1 of every VibeKit build literally
              starts with{" "}
              <code className="font-mono text-[13.5px] rounded border border-[color:var(--border)] bg-[color:var(--bg-subtle)] px-1.5 py-0.5">
                npx create-next-app@latest
              </code>
              . The difference is what happens next: instead of an empty{" "}
              <code className="font-mono text-[13.5px] rounded border border-[color:var(--border)] bg-[color:var(--bg-subtle)] px-1.5 py-0.5">
                app/page.tsx
              </code>{" "}
              staring at you, you have a phase-by-phase plan and an agent that
      ...
      <Script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            description:
              "Comparison of VibeKit framework against the default Next.js scaffolder.",
            author: {
              "@type": "Person",
