    ...
    <div className="rounded-md border border-[#2A221A] bg-[#0E0C09] px-4 py-3.5 font-mono text-[11.5px] leading-[2] space-y-0">
      {lines.map((l, i) => (
        <div key={i} className="flex items-center gap-2">
          <span
            style={{ color: prefixColor[l.type] ?? "#705D4A", minWidth: "1ch" }}
          >
            {prefix[l.type] ?? "·"}
          </span>
  ...
  useGSAP(
    () => {
      gsap
        ...
        .from(
          ".flow-arrow",
          { opacity: 0, x: -6, stagger: 0.14, duration: 0.35 },
          "-=0.7",
        );
    },
    { scope: root },
      ...
      <div className="relative mx-auto max-w-6xl px-4 sm:px-6">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <h2 className="flow-headline font-display mt-6 text-[clamp(2rem,4.5vw,3.25rem)] leading-[1.05] tracking-tight text-[color:var(--text-primary)]">
            One command does{" "}
            <em className="not-italic gradient-text">everything</em>.
          </h2>
          <p className="flow-sub mt-5 text-[16px] leading-relaxed text-[color:var(--text-secondary)]">
            The CLI handles scaffold, idea collection, provider detection, auth
            check, and agent handoff all in the same terminal session.
          </p>
        </div>
...
function FlowCard({
  ...
  return (
    <>
      ...
      {!isLast ? (
        <div
          className="flow-arrow flex items-center justify-center text-[color:var(--text-tertiary)] lg:px-1"
          aria-hidden
        >
